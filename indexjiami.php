...
class indexjiami extends oauth{
	...
	public function __construct($database,$apierrorcode) {
		...
		$this->access_token = $result[0]["access_token"];
		
		$this->jsapi_ticket = $result[0]["jsapi_ticket"];
    ...
    {
        //加密时候，公众账号将会在URL中带上signature、timestamp、nonce、encrypt_type、msg_signature等参数，
		//这些参数用于加解密过程
		$timestamp  = $_GET['timestamp'];
		$nonce = $_GET['nonce'];
