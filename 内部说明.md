...
#### 2 车家互动答题记录（如果游戏长时间不玩，那么客户端应该发起一个finished为0的请求，游戏正常结束在根据完成情况调用该接口）
...
{"code":20000,"message":"successful connected","data":[{"dati":-1,"haoshi1":-1,"timushu":-1,"huarongdao":-1,"haoshi2":-1,"nickname":"","headimgurl":""}]}

| 参数       | 取值范围 | 说明                             |
| ---------- | -------- | -------------------------------- |
| dati       |          | 答题排名，-1为没参与过答题       |
| haoshi1    |          | 答题的耗时，-1为没参与过答题     |
| timushu    |          | 答对的题目数，-1为没参与过答题   |
| huarongdao |          | 华容道的排名，-1为没参与过华容道 |
| haoshi2    |          | 华容道的耗时，-1为没参与过华容道 |
| nickname   |          | 用户的昵称                       |
| headmgurl  |          | 用户的头像                       |
