...
| code   |          | 多次调用该接口的时候，会有可能出现40008,40009的时候，通过异常通知接口告诉我 |
...
#### 2 车家互动答题记录（在游戏开始前先调用一下该接口，finished为0，游戏结束在根据情况调用该接口）
...
| 参数           | 取值范围   | 说明                                 |
| -------------- | ---------- | ------------------------------------ |
| addtion        | int（0-1） | 用户是否完成华容道，1为已完成，0为未 |
| eid            |            | 用户的id                             |
